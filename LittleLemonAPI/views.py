...
from .serializers import CategorySerializer, MenuItemSerializer, CartSerializer, OrderSerializer, UserSerilializer,CartOrderSerializer
...
class CartOrderView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            return CartOrder.objects.filter(order__delivery_crew=user)
        else:
            return CartOrder.objects.filter(cart__user=user)

    def create(self, request, *args, **kwargs):
        cart = Cart.objects.filter(user=request.user)

        if cart.exists():
            total = cart.aggregate(total=Sum('price'))['total']
            ...
            serializer = CartOrderSerializer(data=cart_order_data)
